          ...
          {filteredProducts.map(p => (
            <div key={p.id} className="bg-white border rounded-lg overflow-hidden shawdow-sm">
              ...
              <div className="p-3">
                ...
                <a href={`https://wa.me/${p.whatsapp}?text=$encodeURlComponent('Hi,I'm interested in this product at Sanel Uganda 🇺🇬\n\n`+ '*${p.title}*\n' 'price: ${p.price?.toLocalString()} UGX\n' + 'Category:${p.category}\n'+ 'Image: ${p.images?.[0] || 'No image' } \n\n' 'Is it still available?')}'} target="_blank" className="block w-full text-white text-center py-1.5 rounded text-sm font-medium" style={{ backgroundColor: '#25D366' }}>Contact on WhatsAp </a>
              </div>
            </div>
          ))}
        </div>

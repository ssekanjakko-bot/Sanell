...
export const CATEGORY_OPTIONS = [
    ...
    'Documentary', 'Family', 'Fantasy', 'Horror', '💖Romance', 'Sci-Fi', 'Thriller', 'K-Drama', 'C-Drama', 'DC Movies', 'Marval Movies', 'Trending Now', 'Most popular', 'Most Trending'
]
        ...
        <div className="min-h-screen bg-black text-white pb-20">
                ...
                <section className="px-4 mb-6">
                    ...
                    <div className="flex gap-3 overflow-x-auto">
                        {recentMovies.map((movie) => (
                            <Link key={movie.id} href={`/movies/watch/${movie.id}`} className="flex-shrink-0">
            ...
            <section className="px-4">
                ...
                {movies.length === 0 ? (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        {moviesToShow.map((m) => (
                            <Link key={m.id} href={`/movies/watch/${m.id}`}>
                                <div className="rounded-lg overflow-hidden bg-gray-900 hover:scale-105 transition-transform">
                                    <div className="aspect-[2/3]"><img src={m.posterUrl} alt={m.title} className="w-full h-full object-cover" /></div>
                                    <div className="p-2">
                                        <h3 className="font-semibold text-sm truncate">{m.title}</h3>
                                        <p className="text-xs text-gray-400">{m.year} • {m.category}</p>
                                    </div>
                                </div>
                            </Link>
                        ))}
            ...
            <nav className="fixed bottom-0 left-0 right-0 bg-black/90 backdrop-blur-md border-t border-gray-800">
                <div className="flex justify-around items-center h-16">
                    ...
                    <Link href="/movies" className="flex flex-col items-center text-gray-400"><Tv className="w-6 h-6" /><span className="text-xs">series</span></Link>
                    ...
                    <Link href="/movies" className="flex flex-col items-center text-gray-400"><PlayCircle className="w-6 h-6" /><span className="text-xs">TV Show</span></Link>

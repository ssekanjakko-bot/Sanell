                ...
                <div className="px-3 mt-5">
                    <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
                        {CATEGORY_OPTIONS.map((cat) => (
                            <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-4 py-2 rounded-full text-xs font-bold whitespace-nowrap border ${selectedCategory===cat?'bg-white text-black border-white':'bg-white/10 text-white/70 border-white/10'}`}>{cat}</button>
                        ))}
                    </div>
                </div>

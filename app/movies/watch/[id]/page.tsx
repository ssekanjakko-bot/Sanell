        ...
        <div className="mt-6 grid md:grid-cols-[200px_1fr] gap-6">
          ...
          <div>
            ...
            <p className="text-zinc-400 text-sm mt-1">
              {movie.releaseDate} {movie.duration? `• ${movie.duration}` : ""}
              {genreText? ` • ${genreText}` : ""}
            </p>
            <p className="mt-4 text-zinc-200">{movie.description}</p>
            {movie.director && (
              <p className="mt-3 text-sm">
                <span className="text-zinc-400">Director: </span>{movie.director}
              </p>
            )}
            {movie.cast && (
              <p className="mt-1 text-sm">
                <span className="text-zinc-400">Cast: </span>{movie.cast}
              </p>
            )}
          </div>
        </div>
